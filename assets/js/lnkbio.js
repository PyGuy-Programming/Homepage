...
function checkGenericModalHeight(modal) {
	...
	let threshold = modal.find('.modal-dialog').height();
	if (modal.hasClass('modal-fullscreen')) {
		const content = modal.find('.loaded-container');
		const header = modal.find('.modal-header');
		const measured = (content.length ? content[0].scrollHeight : 0) + (header.length ? header.outerHeight() : 0);
		if (measured) {
			threshold = measured + 40;
		}
	}
